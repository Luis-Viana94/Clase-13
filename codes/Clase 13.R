...
?lm
ols <- lm(formula =  casos_08 ~ treatment + casos_07 , data = covid , subset = is.na(treatment)==F)
ols
summary(ols) 
...
logit = glm(modelo, subset = year == 2020, data = geih, family = binomial(link = "logit") , weights = fex_c_2011) 
logit %>% summary()
